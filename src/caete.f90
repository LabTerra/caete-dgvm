   ...
   subroutine caete_dyn(x,y,run,dt,w0,g0,s0,dcl,dca,dcf,prec,temp,p0,par,rhs&
    ...
    use types
      ...
      use soil_dec, only:  litc   => litter_carbon  ,&
                        &  soic   => soil_carbon    ,&
                        &  p_glob => labile_p_glob  ,&
                        &  n_glob => mineral_n_glob ,&
                        &  gpid   => process_id     ,&
                        &  carb3  => carbon3
      ...
      real(r_4),dimension(3,nt1),intent(out) :: c_soil     ! daily soil carbon KgC m-2
      ...
      real(r_8),dimension(3, npls) :: cleafmes
      real(r_8),dimension(npls) :: cawoodmes,cfrootmes
