...
# REVER O TEMPO DE RESIDÊNCIA DAS RAÌZES FINAS - VARIAR ENTRE 1 mes e 2 anos
    index0 = 0
    rtime = vec_ranging(np.random.normal(0.0, 1.0, r_ceil), 0.07, 9)
    while index0 < diffg:
    ...
    index1 = 0
    rtime_wood = vec_ranging(np.random.normal(0.0, 1.0, r_ceil), 1., 100.)
    while index1 < diffw:
        restime = np.zeros(shape=(3,), dtype=np.float32)
    ...
    # # C : N : P
    # # ----
    leaf_n2c = np.random.uniform(0.0001, 0.05, NPLS)
    awood_n2c = np.random.uniform(0.0001, 0.05, NPLS)
    awood_n2c[0:alloc_g.shape[0]] = 0.0
    froot_n2c = np.random.uniform(0.0001, 0.05, NPLS)

    leaf_p2c = np.random.uniform(0.0001, 0.05, NPLS)
    awood_p2c = np.random.uniform(0.0001, 0.05, NPLS)
    awood_p2c[0:alloc_g.shape[0]] = 0.0
    froot_p2c = np.random.uniform(0.0001, 0.05, NPLS)
