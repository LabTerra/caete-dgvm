...
# REVER O TEMPO DE RESIDÊNCIA DAS RAÌZES FINAS - VARIAR ENTRE 1 mes e 2 anos
    ...
    # # C : N : P
    # # ----
    leaf_n2c = np.random.uniform(0.01, 0.08, NPLS)
    awood_n2c = np.random.uniform(0.01, 0.08, NPLS)
    awood_n2c[0:alloc_g.shape[0]] = 0.0
    froot_n2c = np.random.uniform(0.01, 0.08, NPLS)

    leaf_p2c = np.random.uniform(0.01, 0.03, NPLS)
    awood_p2c = np.random.uniform(0.01, 0.03, NPLS)
    awood_p2c[0:alloc_g.shape[0]] = 0.0
    froot_p2c = np.random.uniform(0.01, 0.03, NPLS)
